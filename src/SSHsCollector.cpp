...
void SSHsCollector::scanServers(const std::vector<uint8_t> &from, const std::vector<uint8_t> &to) {
...
//            size_t cache[4] = {rsaServerCount, dssServerCount, ecdsaServerCount, eddsaServerCount};
            for (int i2 = ipv[2]; i2 < to[2]; ipv[2] = ++i2) {
                for (int i3 = ipv[3]; i3 < to[3]; ipv[3] = ++i3) {
                    auto *sshCollector = new SSHCollector(ipv);
                    if (!sshCollector->checkIPVector()) {
                        delete sshCollector;
                        continue;
                    }

                    boost::asio::post(threadPool, [this, sshCollector] {
                        this->scanServer(sshCollector);
                        delete sshCollector;
                    });
                }
            }
...
void SSHsCollector::scanServer(SSHCollector *sshCollector) {
